...
| PlayerEquipmentState | `Systems/PlayerEquipmentState.cs` | BotDiverAI'nin gear-fear hedefi. `KnockOffMask()` → `OxygenSystem.ApplyZoneEffect`/`RemoveZoneEffect` ile geçici O2 tüketim artışı; `PullRegulator()` → anlık O2 kaybı (`RefillFromSpareTank` negatif) |
| RhythmRecoveryQTE | `Systems/RhythmRecoveryQTE.cs` | Maske/regülatör takma için genel amaçlı ritim mini-oyunu — N beat, her biri için kısa vuruş penceresi (F), kaçırılan vuruş asla dizi'yi başarısız etmez, sadece `OnSequenceCompleted(missedCount)` ile dışarıya bildirilir (çağıran taraf ekstra O2 maliyeti uygular). **Henüz `PlayerEquipmentState`'e bağlanmadı** — `MaskPickup` ile birlikte gelecek entegrasyonu bekliyor |
