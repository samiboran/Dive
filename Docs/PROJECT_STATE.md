...
| Grid Inventory | `Systems/InventorySystem.cs` | Grid footprint (`SO_ItemData.GridWidth/GridHeight` kadar hücre), `occupancy` bool grid ile yerleşim kontrolü, E ile pickup / Q ile drop, stack'leme, `UseSelectedConsumable()` ile Bandage/AdrenalineShot/SpareTank'ı InjurySystem/PanicState/OxygenSystem'e delege eder, `ConsumeSlot()` (drop spawn'sız tüketim — LiftBagDeployer kullanır), `ClearAll()`, `GetTotalWeight()`. **FIX:** `TryAdd()` artık atomic — eskiden mevcut stack'lere sığmayan fazlalık tek bir MaxStack'li slota konup gerisi sessizce kayboluyordu (yine de `true` dönerek); şimdi önce occupancy'nin geçici kopyasında "hepsi sığar mı?" hesaplanıyor, sığmıyorsa envanter HİÇ değişmeden `false` dönüyor, sığıyorsa gerçekten uygulanıyor (`StashSystem.WithdrawTo()`'nun "false = hiçbir şey değişmedi" varsayımıyla tutarlı, aksi halde çiftleme riski vardı) |
...
| StashSystem | `Systems/StashSystem.cs` | Kalıcı depo, tier upgrade (20→40), `Slots` UI hook, `WithdrawTo()`, JSON save/load (`ItemDatabase` + `SaveSystem` üzerinden, her değişimde otomatik kayıt). **FIX:** `TryStore()` taşan miktarı artık MaxStack'e göre birden fazla slota bölüyor — eskiden tek slota `Count` sınırsız (MaxStack'i aşarak) yazılıyordu, bu slot sonra `WithdrawTo()` ile `InventorySystem.TryAdd()`'e verilince yukarıdaki overflow bug'ıyla birleşip veri kaybını katlıyordu |
...
- `InventorySystem.selectedSlot` bir index — `slots` List'i olduğu için başka bir item drop/consume/kaybolunca (ör. LiftBagDeployer, bandaj kullanımı) tüm sonraki index'ler kayar. Oyuncu "3. slotu" seçtikten sonra listenin başka yerinde bir değişiklik olursa, sonraki Q/consumable aksiyonu farklı bir item'ı hedefleyebilir. Küçük envanterlerde (3x2) düşük ihtimal ama gerçek bir risk — index yerine slot referansı/kimliği tutmak gerekebilir
